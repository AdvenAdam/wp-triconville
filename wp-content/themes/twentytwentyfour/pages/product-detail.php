...
if (function_exists('geoip_detect2_get_info_from_ip')) {
    $ip = get_client_ip();
    $location = geoip_detect2_get_info_from_ip($ip);
    echo 'IP: ' . esc_html($ip);
    echo '<br>' . esc_html($location->country->name);

    if ($location && isset($location->country->name)) {
        echo 'Country: ' . esc_html($location->country->name);
    } else {
...
<div class="full-screen w-full"
    ...
    <div class='bg-black bg-opacity-10 h-full w-full flex items-center justify-center'>
        <h1 class="text-3xl lg:text-5xl font-medium text-center text-white capitalize"
            data-aos="fade-up"
            data-aos-once="true"
            data-aos-duration="1000">${productName}</h1>
    </div>
</div>`);
...
<div class="flex justify-center">
    <img src="${validBaseImgUrl}"
         alt="${res.name}"
         class="w-[100vw] my-10 md:w-[80vw] h-[30vh] md:h-[50vh] object-cover px-8" />
</div>
`)
    } else {
        $('#product__header__image').append(`
<div class="flex justify-center ">
    <img src="${validBaseImgUrl}"
         alt="${res.name}"
         class="w-[70vw] md:w-[50vw] lg:w-auto h-fit object-cover px-8" />
</div>
`)
    }
    ...
    $('#product__description').append(`
                <div class="flex sm:flex-row flex-col gap-2 justify-center lg:justify-start mt-10">
                    ...
                    `<a href="${res.collection_sheet}"
                       target="_blank"
                       class="btn-ghost-dark uppercase flex items-end justify-center gap-2">
                        <p class="text-white text-xs">download collection sheet</p>
                        <svg xmlns="http://www.w3.org/2000/svg"
                             fill="none"
                             viewBox="0 0 24 24"
                             stroke-width="1.5"
...
function renderMaterial(res) {
    ...
    if (res.option1 && Array.isArray(res.option1) && res.option1.length > 0) {
        ...
        res.option1.forEach((opt, index) => {
            ...
            $('#option_1').append(
                `<div class="group option_1 cursor-pointer relative ${active? 'active' : ''}"
                     id="${opt.code}"
                     onClick="changeSwatch(1,'${opt.code}')">
                    <div id="tooltip-${opt.code}"
                         class=" absolute -top-16 lg:-left-12 w-fit z-10 invisible group-hover:visible inline-block bg-gray-900 rounded-lg shadow-sm opacity-0 group-hover:opacity-100 ">
                        <p class="px-3 py-2 font-medium text-white w-[140px] sm:w-[180px] sm:line-clamp-2">${opt.name}</p>
                    </div>
                    <img src="${opt.img_link}"
                         class="w-16 md:w-20 h-16 md:h-20 object-contain border-triconville-blue ${active?'border-3':''}" />
                </div>`
            );
        })
    }
    ...
    if (res.option2 && Array.isArray(res.option2) && res.option1.length > 0) {
        ...
        res.option2.forEach((opt, index) => {
            const active = index === 0;
            $('#option_2').append(
                `<div class="group option_2 cursor-pointer relative ${active? 'active' : ''}"
                     id="${opt.code}"
                     onClick="changeSwatch(2,'${opt.code}')">
                    <div id="tooltip-${opt.code}"
                         class=" absolute -top-16 lg:-left-12 w-fit z-10 invisible group-hover:visible inline-block bg-gray-900 rounded-lg shadow-sm opacity-0 group-hover:opacity-100 ">
                        <p class="px-3 py-2 font-medium text-white w-[140px] sm:w-[180px] sm:line-clamp-2">${opt.name}</p>
                    </div>
        ...
        $('#specification__section').append(`
                ...
                <div class="w-full lg:w-[60%] lg:ps-3 xl:ps-5">
                    ...
                    <div id="product__download__container">
                        ...
                        <p class="text-[#798F98] mt-5">Please <span class="underline"><a href="https://indospaceb2b.com/"" target="
                                   _blank">login</a></span> to access all downloadable contents</p>
                    </div>
                </div>
                `)
    }
                       ...
                       class="text-slate-700 text-sm">
                        <div class='flex group items-center gap-2'>
                            <svg xmlns="http://www.w3.org/2000/svg"
                                 ...
                                 stroke-width="1.5"
                                 stroke="currentColor"
                                 class="size-5 pb-1 group-hover:text-slate-400">
                                <path stroke-linecap="round"
                                      stroke-linejoin="round"
                                      d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3" />
                            </svg>
